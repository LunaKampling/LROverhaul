    ...
    public class TrackInfoWindow : DialogBase
    {
        ...
        private void PopulateSong(ControlBase parent)
        {
            ...
            help.Clicked += (o, e) =>
            {
                ...
                $"LRO can sync songs with tracks you make.\nSongs are imported from the \"{Constants.SongsFolderName}\" folder.\n\nWe load files as .ogg, but support importing all common filetypes. LRO will mark files it needs to automatically convert with [convert]",
                "Help", true);
            };
            IsHiddenChanged += (o, e) =>
            {
